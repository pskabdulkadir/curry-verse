  ...
  announcements: [
    {
      ...
      content:
        "Kutbul Zaman - Manevi Rehberim sistemine hoş geldiniz. Ruhsal ve finansal gelişim yolculuğunuz ba��lıyor.",
      type: "info",
      priority: "high",
      targetAudience: "all",
      startDate: new Date(),
...
class MLMDatabase {
  ...
  // Distribute product commissions (40% distributed, 60% to system)
  async distributeProductCommissions(purchaseId: string): Promise<void> {
    try {
      await this.db.read();

      const purchase = this.db.data.productPurchases.find(
        (p: ProductPurchase) => p.id === purchaseId
      );
      ...
      const distributionAmount = totalAmount * 0.4; // 40% for distribution
      const systemFund = totalAmount * 0.6; // 60% for system

      // Commission breakdown (out of the 40% distribution)
      const sponsorBonus = totalAmount * 0.1; // 10% of total
      const careerBonus = totalAmount * 0.25; // 25% of total
      const passiveIncome = totalAmount * 0.05; // 5% of total
      ...
      // If there's a sponsor, distribute commissions through the network
      if (purchase.sponsorId) {
        // Use existing commission distribution system
        await this.calculateAndDistributeCommissions(distributionAmount, purchase.sponsorId);

        // Log recipients for tracking
        const sponsor = await this.getUserById(purchase.sponsorId);
        if (sponsor) {
          commission.recipients.push({
            userId: sponsor.id,
            type: "sponsor",
            amount: sponsorBonus,
          });
        ...
        // No sponsor - all distribution goes to admin/system
        const admin = await this.getUserByEmail("psikologabdulkadirkan@gmail.com");
        if (admin) {
          admin.wallet.balance += distributionAmount;
          await this.updateUser(admin.id, { wallet: admin.wallet });
      ...
      if (admin) {
        ...
        await this.createTransaction({
          ...
          type: "bonus",
          amount: systemFund,
          description: `Ürün satış sistem fonu: ${purchase.productId}`,
          status: "completed",
        });
      }
      ...
      console.log(`Product commission distributed for purchase ${purchaseId}: $${distributionAmount} to members, $${systemFund} to system`);
    } catch (error) {
      console.error("Error distributing product commissions:", error);
    }
  }

  // Get product purchases for a user
  async getUserProductPurchases(userId: string): Promise<ProductPurchase[]> {
    await this.db.read();
    return this.db.data.productPurchases.filter(
      (purchase: ProductPurchase) => purchase.buyerId === userId
    );
  }
  ...
  async getUserProductPurchases(userId: string): Promise<ProductPurchase[]> {
    try {
      ...
      return this.db.data.productPurchases.filter(
        (purchase: ProductPurchase) => purchase.userId === userId
  ...
  // Create product purchase and track activity
  async createProductPurchase(purchaseData: {
    userId: string;
    productId: string;
    productName: string;
    ...
    try {
      ...
      const user = await this.getUserById(purchaseData.userId);
      if (!user) {
        return {
          success: false,
      ...
      const purchase: ProductPurchase = {
        ...
        sponsorId: purchaseData.sponsorId || user.sponsorId,
      ...
      // Update user activity - this counts towards monthly/yearly activity
      const updatedUser = {
        ...user,
        lastActivityDate: new Date(),
        daysSinceLastActivity: 0,
        monthlyActivityStatus: "active" as const
      };

      await this.updateUser(purchaseData.userId, updatedUser);

      // Distribute commissions
      await this.distributeProductCommissions(purchase);
      ...
      return {
        success: true,
        purchase,
        message: "Ürün alışverişi başarıyla kaydedildi."
      };
    } catch (error) {
      console.error("Error creating product purchase:", error);
      return {
        success: false,
        message: "Ürün alışverişi kaydedilirken hata oluştu."
      };
    }
  }
